...
class GaugeSeekBar : View {
    ...
    private companion object {
        ...
        private const val DEFAULT_TRACK_WIDTH_DP = 13
    }
    ...
    override fun onDraw(canvas: Canvas?) {
        canvas?.apply {
            trackDrawable?.draw(this, isThumbOutside)
            progressDrawable?.draw(this, progress, isThumbOutside)
            thumbEntity?.draw(canvas, progress, isThumbOutside)
        }
